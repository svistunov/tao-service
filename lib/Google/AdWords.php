...
///   <constants>
  const VERSION = '0.1.1';
  const NS = "https://adwords.google.com/api/adwords/cm/v200909";
  const FMT_DATE = '%Y%m%d';
  static protected $SERVICE_SUFFIXES = array('job' => 'job', 'idea' => 'o');
...
///     <body>
  static public function wsdl_for($name, $sandbox = false) {
    ...
    preg_match('{_(?<end>[^_]+)$}', $name, $m);
    $suffix = isset(self::$SERVICE_SUFFIXES[$m['end']]) ?
      self::$SERVICE_SUFFIXES[$m['end']] : self::DEFAULT_SERVICE_SUFFIX;

    return 'https://'.($sandbox ? 'adwords-sandbox' : 'adwords').
      '.google.com/api/adwords/'.$suffix.'/v200909/'.
...
///     </args>
///     <body>
  static public function Selector(array $attrs = array()) { return new Service_Google_AdWords_Selector($attrs); }
///     </body>
///   </method>
...
///     <body>
  protected function build_soap_client($wsdl, $options= array(), $headers = array()) {
    ...
    $soap->__setSoapHeaders(new SoapHeader(Service_Google_AdWords::NS, 'RequestHeader', $headers));
    return $soap;
  }
///     </body>
///   </method>
...
///     </args>
///     <body>
  public function get(Service_Google_AdWords_Selector $selector = null) {
    $selector = $selector ? $selector : Service_Google_AdWords::Selector();
    $result = $this->soap->__soapCall(
      'get',
      array(array('selector' => $selector->for_soap())),
      array(),
      array(),
      $out_headers
    );
